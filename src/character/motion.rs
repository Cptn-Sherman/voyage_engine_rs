...
pub fn update_player_motion(
    ...
    for (camera_transform, _, _) in camera_query.iter() {
        for (mut vel, mut motion, mut stance, _) in &mut query {
            ...
            vel.x = motion.movement_vec.x;
            vel.z = motion.movement_vec.z;
        }
    }
...
pub fn apply_spring_force(
    ...
    let spring_force =
        (spring_offset * config.ride_spring_strength) - (-velocity_y * config.ride_spring_damper);
